...
import { BarChart3, FileClock, LogOut, Menu, PenLine, Send, Settings, Sparkles, X } from "lucide-react";
...
const navItems = [
  ...
  { href: "/publishing", label: "发布中心", icon: Send },
...
export function AppShell({ children, email }: { children: React.ReactNode; email?: string }) {
  ...
  const navigation = <>
    <div className="flex h-16 items-center gap-3 border-b px-5"><div className="flex size-9 items-center justify-center rounded-xl bg-primary text-primary-foreground"><Sparkles className="size-4" /></div><span className="font-semibold tracking-tight">AI 内容发布中心</span></div>
  ...
  return <div className="min-h-screen bg-muted/20"><aside className="fixed inset-y-0 left-0 hidden w-60 flex-col border-r bg-sidebar lg:flex">{navigation}</aside><header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b bg-background/95 px-4 backdrop-blur lg:hidden"><div className="flex items-center gap-2 font-semibold"><Sparkles className="size-4 text-primary" />AI 内容发布中心</div><Button variant="ghost" size="icon" onClick={() => setMobileOpen((value) => !value)} aria-label="打开导航">{mobileOpen ? <X /> : <Menu />}</Button></header>{mobileOpen ? <aside className="fixed inset-y-0 left-0 z-30 flex w-72 flex-col border-r bg-sidebar shadow-xl lg:hidden">{navigation}</aside> : null}<main className="min-h-screen lg:pl-60">{children}</main></div>;
}
