...
export async function listProviderConfigs(userId: string): Promise<SafeProviderConfig[]> {
  const { data, error } = await (await createClient()).from("ai_provider_configs")
    .select("id,category,provider,model,quality_model,base_url,api_key_encrypted,enabled")
    .eq("user_id", userId).order("category").order("updated_at", { ascending: false });
  if (error) throw error;
  return ((data ?? []) as ConfigRow[]).map((row) => ({
    id: row.id, category: row.category, provider: row.provider, model: row.model, qualityModel: row.quality_model ?? undefined,
...
export async function getEnabledProviderConfig(userId: string, category: ProviderCategory): Promise<ProviderRuntimeConfig | null> {
  const { data, error } = await (await createClient()).from("ai_provider_configs")
    .select("provider,model,quality_model,base_url,api_key_encrypted").eq("user_id", userId)
    .eq("category", category).eq("enabled", true).maybeSingle();
  if (error) throw error;
  if (!data) return null;
  return { provider: data.provider as ProviderName, model: data.model, qualityModel: data.quality_model ?? undefined, baseUrl: data.base_url, apiKey: decryptProviderSecret(data.api_key_encrypted) };
}
